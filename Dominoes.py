import random
msg = ["Computer is about to make a move. Press Enter to continue...", "It's your turn to make a move. Enter your command.", "Invalid input. Please try again.", "The game is over. {winner}!", "Illegal move. Please try again."]
def start(n):
    ...
    while True:
        ...
        if len(player) != len(computer):
            ...
            break
             ...
             max_ = [max(i) if len(i) != 0 else i for i in [list(filter(lambda x: x[0] == x[1], player)), list(filter(lambda x: x[0] == x[1], computer))]]
             if max(max_) != []:
...
def game():
    ...
    result = 0
    if not (len(computer) and len(player)):
        result = "You won"
        if len(player) == len(computer):
            result = "It's a draw"
        elif len(player):
